...
import { DecodersRegistryService } from '@app/core/services/decoders-registry.service';
...
describe('PresentationsResultsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    const decodersSpy = jasmine.createSpyObj('DecodersRegistryService', ['decoderOf']);
    decodersSpy.decoderOf.and.returnValue({
      decode: jasmine.createSpy('decode').and.returnValue(of({ kind: 'single', attestation: 'decoded', format: AttestationFormat.MSO_MDOC })),
      supports: (format: AttestationFormat) => format === AttestationFormat.MSO_MDOC,
    });
